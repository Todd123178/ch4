...
public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) { //7
 Log.w(ContactDBHelper.class.getName(),"Upgrading database from version " + oldVersion + " to "+ newVersion + ", That data will be safe" );
db.execSQL( "ALTER TABLE contact ADD BFF int" );
onCreate(db);
}
}
